...
export default class NotifyAxosModule extends Vue {
  ...
  created(): void {
    ...
    private async getDataIps(filter = ''):Promise<oneCardInfo[]> {
      const data = (
        await getAxosModuleIp({ filter: filter })
      ).data
    ...
    private async getDataResult(pageNum: number, filter = ''):Promise<oneCardInfo[]> {
      const data = (
        await getAxosModule({ pageNum: pageNum, eachFetch: this.oneShowCount, filter: filter })
      ).data
    ...
    // CsvExportor.downloadCsv(tableData, { header }, 'test.csv')
      ...
      try {
        while (true) {
          data = (
            await getAxosModule({ pageNum: pageNum, eachFetch: this.oneShowCount })
          ).data
      ...
      if (csvData.length > 0) {
        CsvExportor.downloadCsv(csvData, { header }, 'NotifyAxosModule.csv')
        this.$message({
          showClose: true,
    ...
    // CsvExportor.downloadCsv(tableData, { header }, 'test.csv')
      ...
      if (csvData.length > 0) {
        this.downloadText(csvData, header, 'NotifyAxosModule.txt')
      }
      this.dialogVisible = false
    }
